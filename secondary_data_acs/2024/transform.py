...
def hierfindal(categories):
    ratios = categories.div(categories.sum(axis=1), axis=0) # Calculate the percents
...
def pct_hs_diploma_ged_plus(df):
    return (
        (
            df["population_ge_25_with_post_hs_diploma_ged"]
...
def pct_oo_spending_lt_30(df):
    return (
        df['oo_hh_spending_lt_30']
        / df['owner_occupied_households']
    )


def pct_ro_spending_lt_30(df):
    return (
        df['ro_hh_spending_lt_30']
        / df['renter_occupied_households']
    )


def pct_over_20_in_lf(df):
    return (
        df['num_over_20_in_labor_force']
        / df['population_over_20']
    )


def pct_16_to_19_in_lf(df):
    return (
        df['num_16_to_19_in_labor_force']
        / df['population_16_to_19']
    )
...
def pct_children_below_pov(df):
    return (
        df['num_children_below_pov']
        / (
            df['num_children_below_pov'] 
            + df['num_children_above_pov']
        )
    )


districts = pd.read_csv("tracts_to_council_districts_20240725.csv")[["geoid", "district"]]
city_wide = pd.read_parquet(f"../input/nvi_citywide_{YEAR}.parquet.gzip").reset_index()
tract_level = pd.read_parquet(f"../input/nvi_tracts_{YEAR}.parquet.gzip").reset_index()
...
assert len(result) == 7
result.to_excel(f"nvi_districts_{year}_20240805.xlsx")
...
assert len(result) == 1
result.to_excel(f"nvi_citywide_{year}_20240805.xlsx")
