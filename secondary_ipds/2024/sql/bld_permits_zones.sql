...
WITH building_permits AS (
    ...
    INNER JOIN nvi.neighborhood_zones AS zones
        ON
            ST_INTERSECTS(
                ST_TRANSFORM(zones.geometry, 4326),
...
SELECT
    ...
    (
        ...
        * 10000.0
        / NULLIF(zone_population.total_pop, 0)
    ) AS building_permits_per_10000
FROM zone_population
LEFT JOIN building_permits permits
    ON zone_population.zone_id::TEXT = permits.zone_id;
