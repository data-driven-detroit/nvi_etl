"""Shared aggregation functions for indicator compilation.

Used by ACS, MSC, IPDS, and other secondary data tasks.
"""
...
def build_median_calc(strategy):
    def inner(df):
        return pd.to_numeric(df.apply(strategy, axis=1), errors="coerce").round(0)

    return inner


def compile_indicators(indicators: pd.DataFrame, logger):
    """Build aggregation functions for each indicator based on its type."""
    aggregations = {}
    for _, indicator in indicators.iterrows():
        if indicator["indicator_type"] == "count":
            ...
            if indicator["indicator"] == "hierfindal":
                ...
                logger.error(f"'{indicator['indicator']}' doesn't have an assigned aggregator: SKIPPING")
                continue
